...
void Slot::textures_init() {
    if(!slot_texture.loadFromFile("../include/pictures/slot.png")) { // incarca textura
...
unsigned int Slot::get_slot_width() const{
    return slot_texture.getSize().x;
}
unsigned int Slot::get_slot_height() const{
    return slot_texture.getSize().y;
}
