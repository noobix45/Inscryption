...
Player::Player(std::string name_, const int id_,const sf::Font& font_) : name{std::move(name_)}, id{id_}, blood{0}, bones{0},
...
void Player::init_textures()
{
    ...
    {
        std::cout << "Unable to load blood.png\n";
    }
...
void Player::update_numbers(sf::RenderWindow &window)
{
    ...
    blood_text.setOrigin(blood_text.getGlobalBounds().width / 2, blood_text.getGlobalBounds().height / 2); // originea in centru
