        ...
        while (window.pollEvent(event))
        {
                ...
                if(event.type ==  sf::Event::MouseButtonPressed && event.mouseButton.button == sf::Mouse::Left)
                {
                    ...
                    else
                    {
                        ...
                        {
                             // debugg std::cout<<"from place in board: "<<*selected_card;
...
void Game::select_card(const sf::Vector2i mousePos,const int id, Card *&selected_card) // vreau sa modific selectia
{
    if(id==1)
    {
        Card *temp_card = go_through_deck(mousePos, player1.whose_deck()); //ia deck-ul playerului care apeleaza// un pic unintuitive get_deck era deja luat ;)
